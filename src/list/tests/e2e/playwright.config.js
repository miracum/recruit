...
export default defineConfig({
  ...
  // shouldn't try to start anything itself. Locally, launch the app directly --
  // `KEYCLOAK_DISABLED` matches the value set for the `list` service in
  // tests/e2e/docker-compose.yaml so both paths run against the same unauthenticated view.
  webServer: process.env.BASE_URL
    ? undefined
    : {
        command: "dotnet run --project ../../src/Recruit.List.Web",
        env: { KEYCLOAK_DISABLED: "true", PORT: "8080" },
        url: baseURL,
